...
postscript("dob_qpc.eps",height=6,width=12,paper="special",horizontal=FALSE,colormodel="cymk")
#png("Qpc_results.png",height=400,width=800)

#par(mar=c(8,15,5,3), xpd=TRUE, mfrow=c(1,3), cex.axis=1.5, cex.lab=1.5)
par(mar=c(11,17,7,3), xpd=TRUE,  cex.axis=1.8, cex.lab=1.8)
...
image(allpvals[1:4,], col=c(mycols,'white'), xaxt="n", yaxt="n", bty="n", breaks=c(0,0.001,0.01,0.05,0.1,1), xlab = "PC")
axis(1, at=seq(0,1, length=4), label=F)
#mysigpcs = (1:36)[apply(myqvals, 1, min) < 0.1]
axis(1, at=seq(0,1, length=4), label=1:4, las=1)
...
xcords = seq(0,1,length=4)

for (x in 1:4){ #read through PCs
...
mtext('A', side=3, adj=-.5, cex=2, line=2)

### B  example of kernel pc1
par(mar=c(6,5,5,2), xpd=FALSE)
plot(fgmerge$X1, fgmerge$TotalKernelNo, bty="n", xlab = "", ylab = "", yaxt="n", col=as.factor(fgmerge$Subpopulation), lwd=2, ylim = c(130,420))
axis(2, las=2)
mtext("Kernel Number", side=2, line=4, cex=1.5)
mtext("PC 1", side=1, line=4, cex=1.5)
...
mtext('B', side=3, adj=-.4, cex=2, line=0)

### C example of flowering time pc 7
par(xpd=F)
plot(fgmerge$X1, fgmerge$DaysToSilk, bty="n", xlab = "", ylab = "", yaxt="n", col=as.factor(fgmerge$Subpopulation), lwd=2)
axis(2, las=2)
mtext("Days to Silk", side=2, line=4, cex=1.5)
mtext("PC 1", side=1, line=4, cex=1.5)
...
mtext('C', side=3, adj=-.4, cex=2, line=0)

par(mar=c(0,0,0,0))
plot(1, type="n", axes=FALSE, xlab="", ylab="")
legend('top', nicepops, bty="n", pch=1, pt.lwd=2, col = palette(), cex=2, ncol=3)
