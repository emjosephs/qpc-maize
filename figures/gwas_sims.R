...
names(eurodf50) = c('truepos','falsepos','trueneg','falseneg','cor','pval')
...
plot(jitter(c(rep(0.2,200),rep(0.4,200), rep(0.6,200),rep(0.8,200)), factor=0.5),c(amesdf$truepos+amesdf$falsepos,eurodf$truepos+eurodf$falsepos, 
                                                                                   amesdf50$truepos+amesdf50$falsepos, eurodf50$truepos+eurodf50$falsepos), 
     xlim = c(0,1), bty="n", ylim = c(0,3000),
     ylab = "", xlab = "", yaxt = "n", xaxt="n", col = mycol[2])
points(c(0.2, 0.4,0.6,0.8), c(mean(amesdf$truepos+amesdf$falsepos), mean(eurodf$truepos+eurodf$falsepos), mean(amesdf50$truepos+amesdf50$falsepos),mean(eurodf50$truepos+eurodf50$falsepos)), cex=2, lwd=4, pch=16)
axis(2, las=2)
...
mtext('number of significant SNPs found in GWAS', side=2, line=6, cex=1.5)
...
plot(jitter(c(rep(0.2,200),rep(0.4,200), rep(0.6,200),rep(0.8,200)), factor=0.5),c(amesdf$truepos/(amesdf$truepos+amesdf$falsepos),
                                                                                   eurodf$truepos/(eurodf$truepos+eurodf$falsepos), 
                                                                                   amesdf50$truepos/(amesdf50$truepos+amesdf50$falsepos), 
                                                                                   eurodf50$truepos/(eurodf50$truepos+eurodf50$falsepos)), 
     xlim = c(0,1), bty="n",
     ylab = "", xlab = "", yaxt = "n", xaxt="n", col = mycol[2])
points(c(0.2, 0.4,0.6,0.8), c(mean(amesdf$truepos/(amesdf$truepos+amesdf$falsepos)),
                                                             mean(eurodf$truepos/(eurodf$truepos+eurodf$falsepos)), 
                                                             mean(amesdf50$truepos/(amesdf50$truepos+amesdf50$falsepos)), 
                                                             mean(eurodf50$truepos/(eurodf50$truepos+eurodf50$falsepos))), cex=2, lwd=4, pch=16)
axis(1, at = c(0.2,0.4,0.6,0.8), labels = c('Ames 500', 'Euro 500','Ames 50','Euro 50'), cex=2, las=2)
axis(2, las=2)
mtext('Proportion of GWAS hits that are true positives', side=2, line=6, cex=1.5)
