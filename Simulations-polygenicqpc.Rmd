...
#line1 = causal.sites[1,2708:2988]
#line2 = pruned.sites[1,2708:2988]
#getLD <- function(line1, line2){
#  mydf = data.frame(site1 = as.numeric(line1), site2 = as.numeric(line2), stringsAsFactors = F) #each row is an individual
#  
#  D = x1*x4 - x2*x3
#}

gwasStats <- function(myI, gwasPrefix = '../polygenic-maize/data/simFiles/gemmaout.', prunedPrefix = '../polygenic-maize/data/simFiles/sigSnps.ames.', causalN = 500, pcutoff = 0.005){
#read in the GWAS hits
#gemma.out = processGemmaOutput(paste(gwasPrefix,myI,'.assoc.txt',sep=""))
system(paste("head -n 1 ",gwasPrefix,myI,".assoc.txt > ", gwasPrefix,'sig',pcutoff,'.',myI ,sep=""))
system(paste("awk '$13 < ",pcutoff,"' ",gwasPrefix,myI,".assoc.txt >> ", gwasPrefix,'sig',pcutoff,'.',myI ,sep=""))
sig.sites = processGemmaOutput(paste(gwasPrefix,'sig',pcutoff,'.',myI, sep=""))
...
#read in the ldpruned sites
pruned.sites = read.table(paste(prunedPrefix, myI, sep=""), stringsAsFactors = F, header=T)
...
return(c(true.pos, false.pos, false.neg, tot.hits))}

myGwasStats = sapply(1:200, gwasStats)
myGwasStats50 = sapply(1:200, function(x){gwasStats(x, gwasPrefix = '../polygenic-maize/data/simFiles/gemmaout50.', prunedPrefix = '../polygenic-maize/data/simFiles/sigSnps.ames50.',causalN = 50)})
myGwasStats50bonf = sapply(1:200, function(x){gwasStats(x, gwasPrefix = '../polygenic-maize/data/simFiles/gemmaout50.', prunedPrefix = '../polygenic-maize/data/simFiles/sigSnps.ames50.',causalN = 50, pcutoff =  1e-6)})
...
names(gwasStatsdf) = c('truepos','falsepos','falseneg', 'tot.hits')
summary(gwasStatsdf)

gwasStatsdf50 = data.frame(t(myGwasStats50))
names(gwasStatsdf50) = c('truepos','falsepos','falseneg', 'tot.hits')
summary(gwasStatsdf50)

gwasStatsdf50bonf = data.frame(t(myGwasStats50bonf))
names(gwasStatsdf50bonf) = c('truepos','falsepos','falseneg', 'tot.hits')
...
gwasStatsEuro <- function(myI, gwasPrefix = '../polygenic-maize/data/simFiles/gemmaout.euro.', prunedPrefix = '../polygenic-maize/data/simFiles/sigSnps.euro.', causalN=500, pcutoff = 0.005){
#read in the GWAS hits
#gemma.out = read.table(paste(gwasPrefix,myI,'.assoc.txt',sep=""), header=T, stringsAsFactors = F)

system(paste("head -n 1 ",gwasPrefix,myI,".assoc.txt > ", gwasPrefix,'sig',pcutoff,'.',myI ,sep=""))
system(paste("awk '$13 < ",pcutoff,"' ",gwasPrefix,myI,".assoc.txt >> ", gwasPrefix,'sig',pcutoff,'.',myI ,sep=""))
sig.sites = read.table(paste(gwasPrefix,'sig',pcutoff,'.',myI, sep=""), header=T, stringsAsFactors = F)
sig.sites$locus = sapply(sig.sites$rs, function(x){paste('s',strsplit(x, ':')[[1]][1],'_',strsplit(x,':')[[1]][2], sep='')})
...
myGwasStatsEuro50 = sapply(1:200, function(x){gwasStatsEuro(x, prunedPrefix = '../polygenic-maize/data/simFiles/sigSnps.euro.', gwasPrefix = '../polygenic-maize/data/simFiles/gemmaout50.euro.', causalN = 50)})
myGwasStatsEuro50bonf = sapply(1:200, function(x){gwasStatsEuro(x, gwasPrefix = '../polygenic-maize/data/simFiles/gemmaout50.euro.', causalN = 50, pcutoff =  1e-6)})
...
#system('wc -l data/simFiles/sigSnps* > data/simFiles/sigSnpsCounts')
