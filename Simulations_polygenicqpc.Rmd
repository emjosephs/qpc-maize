...
load('data/amesOnly.eig.rda') #this is also from an E matrix

load('data/euro.282.E.rda')
#euroOnly = as.matrix(read.table('data/mergedLandraces.gmriE.5e4.K'))
#euroOnlyeigen = eigen(euroOnly)
#save(euroOnly, euroOnlyeigen, file = "data/euro.only.rda")
...
First, I will simulate quantitative traits with 500 causal loci. These loci will be randomly chosen from across the genome and have effect sizes that are randomly drawn from a normal distribution. Then, pretending that a GWAS has perfectly identified all these loci, I will test for selection with qxpc.

```{r perfect-gwas-sims, eval=F}

#run first once than not again to save time
myI = 141
doPerfectSim <- function(myI, myM=2703, myU = amesEig$vectors, myLambda = amesEig$values, causalPrefix = 'data/simFiles/causalSites.', simPrefix = 'data/simFiles/simTraits.', skiprows = 4){
causalSites = read.table(paste(causalPrefix, myI, sep=""), header=T)
set.seed(myI)
beetas = matrix(rnorm(500), ncol=1, nrow=500) #simulate effect sizes
myG = t(as.matrix(causalSites[,skiprows:ncol(causalSites)])) #get genotypes
m = nrow(myG)
myT = matrix(data = -1/m, nrow = m - 1, ncol = m)
diag(myT) = (m - 1)/m
myGcent = myT %*% myG
myZ = myG%*%beetas #make simulated traits (also these are polygenic scores)
#write.table(myZ, file=paste(simPrefix, myI, sep=""), quote=F, row.names=F, col.names=F) #save the phenotypes for later
#test for selection in the ames panel using the polygenic scores (no conditional test)
myGgeno = myG[1:myM,]
myVa = calcVa(colMeans(myGgeno)/2, beetas)
myZcent = myZ[1:myM] - mean(myZ[1:(myM+1)])
myCms = ((t(myZcent) %*% myU)/sqrt(2*myLambda*myVa))[1,]
return(myCms)
}

#do the simualtions 1000 times
perfectsims1000 <- sapply(1:1000,doPerfectSim)  #rows are individuals, cols are the simulations
save(perfectsims1000, file="data/simFiles/perfect-sims1000")
```


```{r perfect-gwas-sims2}
load('data/simFiles/perfect-sims1000')
mycols = lacroix_palette('Lime', type="continuous", n=5)

#look at variance of Cm across all sims
cmVars = apply(perfectsims1000, 1, var0)
plot(cmVars, bty="n", col = mycols[3], lwd=2, xlab = "PC", ylab = 'var(cM)')
abline(h=1, col=mycols[2], lwd=2)
plot(cmVars[1:500], bty="n", col = mycols[3], lwd=2, xlab = "PC", ylab = 'var(cM)')
abline(h=1, col=mycols[2], lwd=2)

#get cutoffs for test
varexp = amesEig$values/sum(amesEig$values)
sumexp = sapply(1:length(varexp), function(x){sum(varexp[1:x])})
tailCutoff = round(0.9*nrow(perfectsims1000))
pcmax = which(sumexp > 0.25)[1]


#how often will we see evidence of selection?
cmP = apply(perfectsims1000, 2, function(x){ #apply to each simulation
  myPcPs = sapply(x[1:pcmax], function(x){pchisq(var0(x), df=1, lower.tail=F)})
  return(myPcPs)
}) #each row is a PC, col is the p value for that sim

propSig = apply(cmP, 1, function(x){sum(x<0.05)/length(x)})
plot(propSig, bty="n", col = mycols[4], lwd=2, xlab="pc", ylab = "proportion of sims that are significant at p<0.05")
abline(h=0.05, col=mycols[2], lwd=2)
```

### European landraces
```{r perfecteuro, eval=F}

perfectsimseuro = sapply(1:1000, function(x){doPerfectSim(x, causalPrefix = 'data/simFiles/causalSites.euro.', simPrefix = 'data/simFiles/simTraits.euro.', skiprows=6, myM=905, myU =euroOnlyeigen$vectors, myLambda = euroOnlyeigen$values)})
save(perfectsimseuro, file="data/simFiles/perfect-sims-euro1000.rda")

```

```{r perfecteuro2}
load('data/simFiles/perfect-sims-euro1000.rda')
cmVars = apply(perfectsimseuro, 1, var0)
plot(cmVars, bty="n", col = mycols[3], lwd=2, xlab = "PC", ylab = 'var(cM)')
abline(h=1, col=mycols[2], lwd=2)
plot(cmVars[1:500], bty="n", col = mycols[3], lwd=2, xlab = "PC", ylab = 'var(cM)')
abline(h=1, col=mycols[2], lwd=2)

pcm = which(sapply(1:length(euroOnlyeigen$values), function(x){sum(euroOnlyeigen$values[1:x])/sum(euroOnlyeigen$values)}) > 0.3)[1]


cmP = apply(perfectsimseuro, 2, function(x){ #apply to each simulation
  myPcPs = sapply(x[1:pcm], function(x){pchisq(var0(x), df=1, lower.tail=F)})
  return(myPcPs)
}) #each row is a PC, col is the p value for that sim
